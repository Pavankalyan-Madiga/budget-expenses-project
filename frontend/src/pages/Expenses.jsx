...
export default function Expenses({ isDark, toggleTheme }) {
    ...
    useEffect(() => {
        if (payFrom === 'category' && date)
            fetchCategoriesForMonth(date.slice(0, 7));
    }, [date, payFrom]);
    useEffect(() => {
        if (payFrom === 'account' && isOpen)
            fetchAccounts();
    }, [payFrom, isOpen]);
    const handleSave = async (e) => {
        ...
        const attemptSave = async (isRetry = false) => {
            try {
                ...
                setIsOpen(false);
                resetForm();
                if (savedMonth === listMonth)
                    fetchExpenses();
                else
                    setListMonth(savedMonth);
            }
            catch (error) {
    ...
    return (<div className={`flex min-h-screen transition-colors ${isDark ? 'bg-[#0a0a0f] text-white' : 'bg-gray-100 text-black'}`}>
      ...
      <main className="flex-1 p-4 pt-20 sm:p-6 sm:pt-24 lg:p-8 lg:pt-8 relative overflow-x-hidden min-w-0">
        <div className="flex flex-col sm:flex-row sm:justify-between sm:items-center gap-4 mb-8">
          ...
          <div className="flex flex-wrap items-center gap-2 sm:gap-3">
            <div className={`flex items-center space-x-2 px-4 py-2 rounded-lg border text-sm ${isDark ? 'bg-[#12121a] border-gray-800 text-gray-300' : 'bg-white border-gray-300 text-gray-700'}`}>
              ...
              <span className="min-w-[110px] sm:min-w-[140px] text-center text-xs sm:text-sm">{formatMonth(listMonth)}</span>
              <ChevronRight size={16} className="cursor-pointer hover:text-blue-400" onClick={() => changeListMonth(1)}/>
            </div>
            {confirmingClear ? (<div className="flex flex-wrap items-center gap-2 text-sm">
                ...
                <button onClick={handleClearAll} disabled={clearing} className="px-3 py-2 rounded-lg bg-red-600 hover:bg-red-700 transition-colors disabled:opacity-70 flex items-center gap-2">
                  {clearing && (<svg className="animate-spin h-4 w-4" viewBox="0 0 24 24" fill="none">
                      <circle className="opacity-25" cx="12" cy="12" r="10" stroke="currentColor" strokeWidth="4"/>
                      <path className="opacity-75" fill="currentColor" d="M4 12a8 8 0 018-8V0C5.373 0 0 5.373 0 12h4z"/>
                    </svg>)}
                  {clearing ? 'Deleting...' : 'Yes, delete all'}
                </button>
              </div>) : (expenses.length > 0 && (<button onClick={() => setConfirmingClear(true)} className="px-3 py-2 rounded-lg border border-gray-700 text-gray-300 hover:text-white hover:border-gray-500 text-sm transition-colors">
                  Clear All
                </button>))}
        ...
        <div className={`rounded-xl border overflow-x-auto ${isDark ? 'bg-[#12121a] border-gray-800' : 'bg-white border-gray-200 shadow-sm'}`}>
          <table className="w-full min-w-[640px] text-sm text-left">
            ...
            <tbody>
              {!hasLoadedOnce && fetchingExpenses ? (<tr>
                  <td colSpan="5" className="px-6 py-12 text-center text-gray-500">
                    <div className="flex flex-col items-center justify-center gap-3">
                      ...
                      {slowNotice && (<span className="text-xs max-w-xs">
                          Still loading — the server can take up to a minute to wake up on its first request.
                        </span>)}
                    </div>
                  </td>
                </tr>) : expenses.length === 0 ? (<tr><td colSpan="5" className="px-6 py-8 text-center text-gray-500">No expenses found for {formatMonth(listMonth)}.</td></tr>) : (expenses.map((exp) => (<tr key={exp.id} className={`border-b transition-colors ${isDark ? 'border-gray-800 hover:bg-[#1e1e2e]' : 'border-gray-100 hover:bg-gray-50'}`}>
                    <td className="px-6 py-4 font-medium">{exp.description}</td>
                    <td className="px-6 py-4 text-gray-400">
                      {exp.category ? exp.category : (<span className="inline-flex items-center gap-1 text-blue-400">
        ...
        {isOpen && (<div className="fixed inset-0 bg-black/60 flex items-center justify-center z-[100] p-4">
              ...
              <form onSubmit={handleSave}>
                ...
                <div className="mb-4">
                  ...
                  <div className="grid grid-cols-2 gap-2">
                    <button type="button" onClick={() => setPayFrom('account')} className={`py-2.5 rounded-lg text-sm font-medium border transition-colors ${payFrom === 'account' ? 'bg-blue-600 border-blue-600 text-white' : 'bg-[#1e1e2e] border-gray-700 text-gray-400 hover:text-white'}`}>
                      Bank Account
                    </button>
                    <button type="button" onClick={() => setPayFrom('category')} className={`py-2.5 rounded-lg text-sm font-medium border transition-colors ${payFrom === 'category' ? 'bg-blue-600 border-blue-600 text-white' : 'bg-[#1e1e2e] border-gray-700 text-gray-400 hover:text-white'}`}>
                      Others
                    </button>
                  </div>
                </div>

                {payFrom === 'account' ? (<div className="mb-4">
                    <label className="block text-xs text-gray-400 mb-2">ACCOUNT</label>
                    {fetchingAccounts ? (<div className="flex items-center gap-2 text-sm text-gray-500 px-3 py-2">
                        <svg className="animate-spin h-4 w-4" viewBox="0 0 24 24" fill="none">
                          <circle className="opacity-25" cx="12" cy="12" r="10" stroke="currentColor" strokeWidth="4"/>
                          <path className="opacity-75" fill="currentColor" d="M4 12a8 8 0 018-8V0C5.373 0 0 5.373 0 12h4z"/>
                        </svg>
                        Loading accounts...
                      </div>) : accounts.length > 0 ? (<select value={accountId} onChange={(e) => setAccountId(Number(e.target.value))} className="w-full bg-[#1e1e2e] border border-gray-700 rounded-lg px-3 py-2 text-sm focus:outline-none focus:border-blue-500 [color-scheme:dark]" required>
                        {accounts.map((acc) => (<option key={acc.id} value={acc.id}>{acc.name} • {acc.bank} (${acc.balance.toFixed(2)})</option>))}
                      </select>) : (<p className="text-sm text-gray-500 px-3 py-2 border border-gray-700 rounded-lg bg-[#1e1e2e]">
                    ...
                    <div>
                      ...
                      <select value={category} onChange={(e) => setCategory(e.target.value)} className="w-full bg-[#1e1e2e] border border-gray-700 rounded-lg px-3 py-2 text-sm focus:outline-none focus:border-blue-500 [color-scheme:dark]" required>
                        {budgetCategories.length > 0 ? (budgetCategories.map((cat, index) => (<option key={index} value={cat}>{cat}</option>))) : (<option value="" disabled>No budgets set for this month</option>)}
                      </select>
                      {date && (<p className="text-xs text-gray-500 mt-1">
                ...
                {saveSlowNotice && (<p className="text-xs text-gray-500 text-center mb-4">
